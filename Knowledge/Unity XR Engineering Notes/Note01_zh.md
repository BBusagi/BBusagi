...
大致可以拆成下面几层。本文里的层级图统一按从上到下阅读：应用层在上，设备 / Runtime 基础层在下。后面的详细解释仍然会从底层往上讲，因为这样更容易先建立基础概念。
...
| Runtime 层 | Meta Runtime、SteamVR Runtime、WMR Runtime | 管理设备追踪、渲染提交、空间系统和运行环境 |
...
Runtime 可以理解成设备 / 平台侧的 XR 运行环境。

它不是 Unity 包，而更接近系统层：负责设备追踪、帧时序、渲染提交、Compositor 行为、设备状态，以及空间运行服务。

例如 Meta Runtime、SteamVR Runtime 与 Windows Mixed Reality Runtime。
...
这里最重要的是：Runtime 位于 Unity XR Plugin 层之下。

当我们在 Unity 中启用 OpenXR Plugin 时，并不意味着 Quest 不再使用 Meta Runtime。Unity 侧 Plugin 改变的是接入路径，但设备侧仍然运行在 Meta Runtime 系统上。

所以很多时候，即使你在 Unity 中使用的是 OpenXR，最终运行时行为仍然取决于 Meta Runtime 与 Quest 的底层 Tracking、Rendering 与 Device System。
...
这一层位于 Unity 侧，负责“让 Unity 能连接 Runtime”。

更具体地说，Unity XR Plugin 层主要负责：

- 选择并加载正确的 XR Backend。
- 启动和停止 Unity XR 子系统。
- 把 Unity 的渲染、输入、追踪和生命周期逻辑接入 Runtime。
- 通过包设置和 Feature Flags 向 Unity 暴露 Runtime 能力。
- 把 Unity 侧概念转换成面向 Runtime 的 API 调用。
...
| Unity Plugin | 接入方式 | 更适合的场景 |
| --- | --- | --- |
| Oculus XR Plugin | 通过更偏 Meta 私有的路径把 Unity 接入 Meta Runtime | 已经深度绑定 Oculus Integration / OVRInput 的旧 Quest 项目 |
| OpenXR Plugin | 通过 OpenXR 标准接口把 Unity 接入 OpenXR Runtime | 新项目、多平台项目、希望减少厂商绑定的项目 |

在 Quest 上，这两套 Plugin 很多时候都能正常运行，因为它们最终都会接入 Meta Runtime。

这并不意味着有两套 Runtime 并存，而是同一个 Runtime 可以暴露不同的接入路径：Meta 私有接口和 OpenXR 标准接口。
...
Unity 里的 OpenXR Plugin，是 Unity 用来接入 OpenXR Runtime 的包。

本篇先只需要记住一点：XR Plugin 不是 Runtime 本身，而是 Unity Engine 与 XR Runtime 之间的 Unity 侧桥梁层。
